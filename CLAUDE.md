...
A tiny .NET WinForms desktop app that lets non-technical Windows users run the server without a terminal. **Targets .NET Framework 4.8** (built into Windows 10 1903+/11), so the built exe is ~15 KB and needs no runtime install. It is a single main window — **start / stop / restart the server *process*** plus a live log view; it deliberately has **no session-level controls** (create/stop/restart/delete sessions stay in the Android app).
...
- `ServerLocator.cs` — finds the server dir (prefers `<exeDir>/server` for the shipped layout, else walks up parents for dev), and reads `port` from `config.json` for the status line.
- `CCRemoteLauncher.csproj` — `net48`, `WinExe`; references `System.Windows.Forms`/`System.Drawing`; build-only `Microsoft.NETFramework.ReferenceAssemblies` lets the .NET 8 SDK compile net48 with no machine-wide targeting pack. `app.manifest` declares PerMonitorV2 DPI + Win10/11.

### Versioning (`VERSION` + `tools/`)
The repo-root `VERSION` file is the single source of truth for the human-facing version. `node tools/sync-version.mjs` propagates it to `server/package.json` (`version`), `launcher/CCRemoteLauncher.csproj` (`<Version>/<AssemblyVersion>/<FileVersion>`), and `android/app/build.gradle` (`versionName`). **`versionCode` is left untouched** — it must increase monotonically and is bumped manually. `tools/package-win.mjs` builds the shippable `dist/CC-Remote-Server/` (launcher exe + staged `server/` with production-only `node_modules`). Note: root `scripts/`, `package.json`, `package-lock.json` are gitignored, so repo-wide tooling lives in the tracked `tools/`.
...
node tools/package-win.mjs   # build dist/CC-Remote-Server/ (launcher + staged server)
```
