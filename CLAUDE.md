...
| S→C | `session_tool` | `session_id`, `status` (`running`\|`done`), `name` (tool name, on `running`), `id` (tool_use id, optional), `detail` (argument summary e.g. command/file, optional) |
