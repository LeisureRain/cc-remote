...
- `MainForm.cs` — the window. Button bar (启动/停止/重启服务, **设置**, 清空日志, 打开服务端目录) + a horizontal `SplitContainer`: **top = session list** (`ListView`), **bottom = dark monospaced read-only log box** (ring-buffered to ~2000 lines) + status line (running/stopped + port + 会话/客户端 counts). Spawns `node src/index.js` via `Process` with `WorkingDirectory = <serverDir>` (required: server resolves `sessionsDir` from cwd), redirects stdout/stderr into the log. Auto-starts on launch. **Session list:** a `Forms.Timer` polls the server's public `/health` endpoint every 3 s (background `HttpWebRequest`, 2.5 s timeout, parsed with `JavaScriptSerializer`) and renders one row per session — directory name, status (运行中/已暂停/已退出), client count, message count (buffer size), created time, full path. No auth token needed (`/health` is public). The list clears immediately on stop/exit. **设置** opens `SettingsForm` to edit port + workspace, writes them back to `config.json`, and auto-restarts the server to apply. Stop/exit kill the whole process tree via `taskkill /PID <id> /T /F` (net48 has no `Kill(entireProcessTree)`), so child `claude` processes don't leak.
