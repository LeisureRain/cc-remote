...
- `MainForm.cs` — the window. Button bar (启动/停止/重启服务, 清空日志, 打开服务端目录) + dark monospaced read-only log box (ring-buffered to ~2000 lines) + status line (running/stopped + port + version). Spawns `node src/index.js` via `Process` with `WorkingDirectory = <serverDir>` (required: server resolves `sessionsDir` from cwd), redirects stdout/stderr into the log. Auto-starts on launch. Stop/exit kill the whole process tree via `taskkill /PID <id> /T /F` (net48 has no `Kill(entireProcessTree)`), so child `claude` processes don't leak.
- `ServerLocator.cs` — `EnsureServerDir()` extracts the embedded `server-bundle.zip` to `%LOCALAPPDATA%\CC-Remote\server` (skips when a `.bundle-version` stamp matches the exe version; **never overwrites an existing `config.json`** so user edits/port survive upgrades; `sessions/` + `profiles/` persist there too). Falls back to a sibling/ancestor `server/` folder when no bundle is embedded (plain dev build). Also reads `port` from `config.json` for the status line.
