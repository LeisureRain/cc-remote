...
A tiny .NET WinForms desktop app that lets non-technical Windows users run the server without a terminal. **Targets .NET Framework 4.8** (built into Windows 10 1903+/11), so no runtime install is needed. **The entire server is embedded inside the exe** (as `server-bundle.zip`) and extracted on first run, so the shipped distribution is a *single ~160 KB exe* — users download one file. It is a single main window — **start / stop / restart the server *process*** plus a live log view; it deliberately has **no session-level controls** (create/stop/restart/delete sessions stay in the Android app).
- `Program.cs` — entry point; single-instance `Mutex`, then `Application.Run(new MainForm())`.
- `MainForm.cs` — the window. Button bar (启动/停止/重启服务, **设置**, 清空日志, 打开服务端目录) + dark monospaced read-only log box (ring-buffered to ~2000 lines) + status line (running/stopped + port + version). Spawns `node src/index.js` via `Process` with `WorkingDirectory = <serverDir>` (required: server resolves `sessionsDir` from cwd), redirects stdout/stderr into the log. Auto-starts on launch. **设置** opens `SettingsForm` to edit port + workspace, writes them back to `config.json`, and auto-restarts the server to apply. Stop/exit kill the whole process tree via `taskkill /PID <id> /T /F` (net48 has no `Kill(entireProcessTree)`), so child `claude` processes don't leak.
...
- `CCRemoteLauncher.csproj` — `net48`, `WinExe`; references `System.Windows.Forms`/`System.Drawing`/`System.IO.Compression(.FileSystem)`; embeds `server-bundle.zip` (conditional, so a bare `dotnet build` still works). Build-only `Microsoft.NETFramework.ReferenceAssemblies` lets the .NET 8 SDK compile net48 with no machine-wide targeting pack. `app.manifest` declares PerMonitorV2 DPI + Win10/11.
