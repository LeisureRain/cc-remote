...
- `TerminalActivity.java` — Chat-style terminal screen for one session. Uses RecyclerView with Markwon for Markdown rendering (including tables). Sends user input via `send_chat`, streams Claude's reply via `session_delta`, and shows tool-call activity via `session_tool`. **Live activity status bar:** a bar between the chat list and input shows what Claude is doing *right now* (`✳ Thinking…` / `⚙ <tool> · <detail>` / `✍ Writing…`) plus an elapsed-time counter, driven by a 1 s ticker and `lastActivityMs` (bumped on every `session_delta`/`session_tool`/`session_thinking`). After 30 s with no activity it turns amber (`⚠ … no update Ns — tap to interrupt`) so a genuine hang is distinguishable from a long step; the bar (and the Send→Stop button) are active for the *whole* turn, so the user can interrupt during thinking/tool phases, not just while text streams. Tool calls leave a persistent, browsable `⚙ name · detail` line in the transcript (✓-dimmed when done), annotated with a `→ output snippet` line once the tool's result arrives (red on error). Tracks session lifecycle via foreground service callbacks. Loads locally-cached chat history (via `ChatHistoryStore`) on open for instant display, then lets the server's `chat_history` overwrite it as source of truth.
...
A tiny .NET WinForms desktop app that lets non-technical Windows users run the server without a terminal. **Targets .NET Framework 4.8** (built into Windows 10 1903+/11), so no runtime install is needed. **The entire server is embedded inside the exe** (as `server-bundle.zip`) and extracted on first run, so the shipped distribution is a *single ~160 KB exe* — users download one file. It is a single main window — **start / stop / restart the server *process*** plus a live log view and a **read-only session list** (mirroring the Android session list). It deliberately has **no session-level controls** (create/stop/restart/delete sessions stay in the Android app) — the list is for visibility only.
- `Program.cs` — entry point; single-instance `Mutex`, then `Application.Run(new MainForm())`.
- `MainForm.cs` — the window. Button bar (Start/Stop/Restart, **Settings**, Clear Log, Open Server Folder) + a horizontal `SplitContainer`: **top = session list** (`ListView`), **bottom = dark monospaced read-only log box** (ring-buffered to ~2000 lines) + status line (Running/Stopped + port + Sessions/Clients counts). Spawns `node src/index.js` via `Process` with `WorkingDirectory = <serverDir>` (required: server resolves `sessionsDir` from cwd), redirects stdout/stderr into the log. Auto-starts on launch. **Session list:** a `Forms.Timer` polls the server's public `/health` endpoint every 3 s (background `HttpWebRequest`, 2.5 s timeout, parsed with `JavaScriptSerializer`) and renders one row per session — directory name, status (Running/Paused/Exited), client count, message count (buffer size), created time, full path. No auth token needed (`/health` is public). The list clears immediately on stop/exit. **Settings** opens `SettingsForm` to edit port + workspace, writes them back to `config.json`, and auto-restarts the server to apply. Stop/exit kill the whole process tree via `taskkill /PID <id> /T /F` (net48 has no `Kill(entireProcessTree)`), so child `claude` processes don't leak. The entire launcher UI and all launcher/server log output are English-only.
- `SettingsForm.cs` — modal dialog editing the two most-changed server settings: listening **port** (NumericUpDown) and **workspace** directory (textbox + folder picker; empty = no restriction). Other `config.json` fields are left untouched.
- `ServerLocator.cs` — `EnsureServerDir()` extracts the embedded `server-bundle.zip` to `%LOCALAPPDATA%\CC-Remote\server` (skips when a `.bundle-version` stamp matches the exe version; **never overwrites an existing `config.json`** so user edits/port/workspace survive upgrades; `sessions/` + `profiles/` persist there too). Falls back to a sibling/ancestor `server/` folder when no bundle is embedded (plain dev build). `ReadPort`/`ReadWorkspace`/`SaveConfig` read & rewrite just those two fields in `config.json` via regex (MatchEvaluators so Windows path chars like `\` and `$` are preserved/JSON-escaped correctly).
...
```bash
cd launcher
dotnet build -c Release      # -> bin/Release/net48/CCRemoteLauncher.exe (~15 KB)
```
...
**Server:** `ws` (WebSocket), `uuid` (session IDs). Zero native deps — sessions run headless `claude` via `child_process` (no PTY), so the production `node_modules` is pure JS and bundles cleanly.
**Windows Launcher:** .NET Framework 4.8 (system-provided); build-time only `Microsoft.NETFramework.ReferenceAssemblies`. No runtime npm/NuGet deps shipped.
